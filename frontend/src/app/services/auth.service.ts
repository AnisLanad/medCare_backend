...
export class AuthService {
  ...
  login(loginData : any): Observable<any> {
    return this.http.post(`http://127.0.0.1:8000/api/token/`, loginData,{headers: { 'Content-Type': 'application/json' }
    }) .pipe(
        tap((response: any) => {
  ...
  getUserName() {
       ...
       const Name = this.http.get<{ Nom: string; Prenom: string }>(`http://127.0.0.1:8000/api/user/${username}/${role}/`);
       return Name
      }
  ...
  getMedecinPatients() {
        const username = localStorage.getItem('username');
        const Patients = this.http.get<Array<{DPI_ID: number ; Nom: string; Prenom: string; DateNaissance: string; NSS: string}>>(
   ...
   getNbConsultations() {
     const username = localStorage.getItem('username');
     const nbcons = this.http.get<{count:number ; patient1:string ; patient2:string}>(`http://127.0.0.1:8000/api/medecin/${username}/nbcons/`);
     return nbcons
