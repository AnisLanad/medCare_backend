...
export class AuthService {
  ...
  login(loginData: any): Observable<any> {
    return this.http.post(`http://127.0.0.1:8000/api/token/`, loginData, {
  ...
  getMedecinPatients() {
    const username = localStorage.getItem('username');
    const Patients = this.http.get<Array<{
      DPI_ID: number;
      Nom: string;
      Prenom: string;
  ...
  //make a function to get the all the patients
  getAllPatients() {
    return this.http.get<Array<{ DPI_ID: number; Nom: string; Prenom: string; DateNaissance: string; NSS: string }>>(
      `http://127.0.0.1:8000/api/patients/`);// Utilisation des backticks pour l'URL
  }
